mod parse_monster;
use crate::parse_monster::monster;
use clap::Parser;
use dotenv::dotenv;
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    if let Some(s) = cli.url {
        ...
        let monster = monster::Monster::new(&monster_document, &s);
        if monster.validate() {
        ...
        for (i, node) in input.enumerate() {
                ...
                && !href.contains("naiad-queen")
            {
                ...
                let monster = monster::Monster::new(&monster_document, href);
                if monster.validate() {
                    send_db(monster).await?;
                } else {
                    println!("Monster did not validate:\n{}", monster);
...
async fn send_db(monster: monster::Monster) -> Result<(), sqlx::Error> {
